...
app.get('/api/bins', async (req, res) => {
  try {
    const bins = await BinReading.find({}); // get all bins from DB
    res.json(bins);
  } catch (err) {
    console.error('Error fetching bins:', err);
    res.status(500).json({ error: 'Failed to fetch bins' });
